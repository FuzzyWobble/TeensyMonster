...
// EDIT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
  ...
  //......................................................
  //PUSHBUTTON____________________________________________ Max: 24 pusbuttons + 2 for each encoder not used + 1 for each Teensy analog in not used
  ...
  //pins with '9' are used for other purposes and can NEVER be used as pushbuttons 
  ...
  //....................................................
  //ROTARY ENCODER______________________________________ Max: 3 or 6
  ...
  //.........................................................
  //ANALOG IN _______________________________________________ Max: 23
...
int analogThreshold = 3; //threshold - you can adjust this if you have some jitter problems
...
void setup(){ 
  ...
  if(enableDebug){
    ...
    for(int p=0;p<46;p++){ //loop through all pushbuttons
      for(int e=0;e<6;e++){ //loop through all encoders
        if( (encoderPins[e][0]==p && toReadPushButton[p]==1) || (encoderPins[e][1]==p && toReadPushButton[p]==1) ){
          Serial.println("ERROR - pin ["+(String)p+"] found both in pushbutton and encoder");  
        }
      }
    }
    //LED vs encoder
    for(int l=0;l<46;l++){ //loop through all LEDs
      for(int e=0;e<6;e++){ //loop through all encoders
        if( (encoderPins[e][0]==l && toDisplayLED[l]==1) || (encoderPins[e][1]==l && toDisplayLED[l]==1) ){
          Serial.println("ERROR - pin ["+(String)l+"] found both in encoder and LED");  
        } 
      }
    } 
    //pushbutton vs LED
    for(int i=0;i<46;i++){ //loop through all pins
      ...
      Serial.println("ERROR - pin ["+(String)i+"] found both in pushbutton and LED");  
      }
    }
    //teensy analog vs pushbutton
    for(int a=0;a<7;a++){ //loop through all analog
      if(toReadPushButton[a+38]==1  && toReadAnalogTeensy[a]==1){
      Serial.println("ERROR - pin ["+(String)(a+38)+"] found both in pushbutton and analog teensy");
      }
    }
    //teensy analog vs LED
    for(int a=0;a<7;a++){ //loop through all analog
      if(toDisplayLED[a+38]==1  && toReadAnalogTeensy[a]==1){
      Serial.println("ERROR - pin ["+(String)(a+38)+"] found both in LED and analog teensy");
      }
    }
  }
  ...
  if(enableDebug){
    Serial.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
    Serial.println("PROGRAM WILL START LOOPING IN FIVE SECONDS...");
...
void loop(){
  ...
  int j = i + shiftChange; //add the shift change
    if(toReadPushButton[i]==1){ //check if this a pin with a pushbutton hooked up to it
      ...
      if(pushbuttonMode[j]==1 && tempDigitalRead!=buttonState[j]){ //NORMAL MODE (1)
  ...
  for(int i=0;i<16;i++){ //loop through 16 mux channels
    if(toReadAnalogMux[i]==1){ //we read this mux channel analog input
      ...
      if(abs(analogInsPrev[i]-tempAnalogInMux)>analogThreshold){ //ensure value changed more than our threshold
        ...
        midiNoteOnOff('a',tempAnalogInMap,i); //send message
  ...
  for(int i=0;i<7;i++){ //loop through the 6 analog teensy channels
    if(toReadAnalogTeensy[i]==1){ //we read this analog input
      ...
      if(abs(analogInsPrevTeensy[i]-tempAnalogInTeensy)>analogThreshold){ //ensure value changed more than our threshold
        ...
        midiNoteOnOff('a',tempAnalogInMapTeensy,i+16); //send message
  ...
  //you have to write your own LED code, 'cause everyone will expect their LEDs to behave in a different way
  //SAMPLE 1
  ...
  if(buttonState[4]==1){ //push-button on pin 4 is on
  ...
  if(buttonState[12]==1){ //push-button on pin 4 is on
...
void midiNoteOnOff(char type, int val, int pin){ //type (pushbutton, encoder, analog), val(digital on/off, analog val), pin (pin number)
  String clickState;
  switch (type){
